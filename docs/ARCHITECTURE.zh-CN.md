...
5. 按状态间隔输出一行状态 JSON(电平、延迟、引擎指标)。间隔可配置:应用请求
   80 ms,CLI 在 `--status-json`/`--verbose` 下默认 1000 ms。
...
**时钟漂移速率匹配(`output_rate_match`,默认开)。** 管线跑在采集(mic)时钟上,
输出设备按自己的时钟抽取,far 环则由 render/loopback 时钟填充。这几个时钟相互独立、
会漂移,即使标称相同(都是 48 kHz、不发生格式重采样)也一样 —— 虚拟音频设备漂移
最厉害。不补偿的话,漂移会慢慢抽空输出环(underrun → 零填充爆音)或写溢出。pre-T3
的做法是参考环过期就硬丢帧(`skip_stale`)、输出环空了就零填充;两者都可闻,还会
打乱 far/near 对齐、拖垮 AEC。

速率匹配改为用**重采样**吸收漂移,不丢帧。在设备率 == 管线率的快路径上(两率不等
时已有固定比率重采样在跑),PI 控制器(`RateController`)读输出环水位、与 2 帧设定
点比较,输出一个钳到 ±3% 的比值 `trim`(带抗积分饱和),重采样器据此按
`base_ratio · (1 + trim)` 运行,把占用量拉回设定点;far 路径用对应重采样器替代
`skip_stale`。关键是**半帧软死区**:水位误差在 ±½ 帧内时有效误差为 0、`trim` 衰减
回 0,只抖动的设备保持 bit-exact、不被重采样 —— 只有穿出 ½ 帧的持续漂移才拉动比
值。±3% 钳位保证音高偏移不可闻。

...
- **问题**:`echoless-app` 是**独立 workspace**,根 `--workspace` 不会编译它。CI 的 test/clippy/
  build 三步**完全不覆盖** `app/src-tauri/src/lib.rs`(567 行、含全部 shell/IPC 安全面)。其 Rust
  回归只能靠打包步骤间接触发(若有)。
- **修复指令**:CI 增加针对 `app/src-tauri` 的独立 `cargo clippy -D warnings` 与 `cargo build`
  (在该目录单独跑),并将 Tauri 后端纳入 lint gate。
...
**根因(已实证)**:GUI 实际运行的 `echoless` CLI 是**过时的 debug 二进制**,它早于
...
- **库与模型是两个独立产物**。`download_localvqe_model`(`app/src-tauri/src/lib.rs:234`)只下载 **`.gguf`
  ...
  **从 LocalVQE 源码自行编译**——echoless **既不下载、也不打包、也无任何构建步骤**产出它。
  对比:nvafx 有 `download-install` 自动取 runtime;LocalVQE 没有等价物。

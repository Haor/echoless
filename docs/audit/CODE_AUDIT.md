...
> **不含** `vendor/sonora`(第三方 vendored 引擎,只读分析对象)与 `reference_repos/`。
...
| SON-1 | sonora | Low | echoless `let _ =` 吞掉 `process_render/capture` 的 `Result`(错误静默) |
| SON-2 | sonora | Low | 热路径 `assert!`(release 也触发)与 `process_capture` 潜在每帧分配未经 profile |
| SON-3 | sonora | Low | sonora 独立 workspace(edition 2024),未进 echoless CI 的 clippy/audit gate |
...
- **`vendor/sonora`**:按约定为只读第三方,本次未审计其内部正确性/性能;但它是音质核心,建议
...
## 10. Sonora(vendored 引擎)审计

> 范围:`vendor/sonora`,52138 行 Rust / 7 个 crate(`sonora-aec3` 17k、`sonora-agc2` 9k、
> `sonora` facade 12.7k、`sonora-fft` 5k、`sonora-ns` 3.1k、`sonora-simd` 2.7k、`sonora-common-audio` 2.1k)。
> 它是 `dignifiedquire/sonora`(WebRTC AudioProcessing 的纯 Rust 移植,BSD-3-Clause)的 fork,
...
- **无可达生产 panic**:全仓 `panic!`×6 / `unreachable!`×3 经逐一核对,在 echoless 的固定
  ...
  - `pitch_search*.rs` 的 `File::open(...).unwrap_or_else(panic)` 位于 **`#[cfg(test)]`** 模块,
    不进生产二进制。
- **`unsafe` 用法规范**:206 处 `unsafe` 集中在 `sonora-simd` / `sonora-fft` / `matched_filter`
...
- **fork 改动面极小且有据**:唯一对外新增是 `AudioProcessingBuilder::aec3_config()`
  (`crates/sonora/src/audio_processing.rs:373`)+ `set_aec3_config_override`,在 `build()` 的
...
### SON-1 — echoless 吞掉 sonora 的处理结果 Result [Low]
- **位置**:`crates/echoless-processors/src/sonora_aec3.rs:334, 339-342, 344-347`
...
- **位置**:`crates/sonora-aec3/src/*`(`assert!`/`assert_eq!` 与 `debug_assert!` 混用)、
...
- **问题**:
  ...
  2. `echo_remover.rs` 的 `process_capture` 窗口内抽到约 10 处 `vec!`/`with_capacity` 类 token,
     未能在本次预算内确认是否稳态每帧分配。
- **修复指令**:
  1. 审查 `sonora-aec3`/`sonora-ns`/`sonora-agc2` 处理路径上的 `assert!`/`assert_eq!`,把"理论不变量
...
### SON-3 — sonora 不在 echoless CI lint/audit gate 内 [Low]
- **位置**:`vendor/sonora/Cargo.toml`(独立 `[workspace]`,edition 2024)、根 `Cargo.toml`
  `exclude = ["vendor/sonora"]`、`.github/workflows/build.yml`。
- **问题**:sonora 被根 workspace 排除,echoless CI 的 `clippy -D warnings` / 测试 / (建议新增的)
  `cargo audit` 都不覆盖它。它有自带 CI,但 echoless 仓库内对它无任何回归 gate;一旦本地对 fork 再
  改动(如继续开放更多 config 字段),无 CI 拦截。
- **修复指令**:CI 增加一步对 `vendor/sonora` 单独 `cargo test -p sonora`(至少 facade + aec3 关键
  测试)与 `cargo clippy`,确保 fork 改动不回归。

### 未在本次覆盖(sonora)
- **AGC2 / RNN-VAD 路径**(`sonora-agc2`,echoless `agc=true` 时启用)与**立体声 render**
  (`reference_channels="stereo"`)路径未做深度跑测;建议各补一条端到端 smoke。
- **数值一致性**:未与上游 C++ WebRTC 做逐样本对拍(sonora 自带 cpp-validation,但 echoless 侧未跑)。
...
| SON-1 | **仍成立**。`sonora_aec3.rs:334/339/344` 仍 `let _ =` 吞 process 结果 |
...
- **结论**:**禁止 clean-room 重写**。本仓库**架构基本健全**——crate 职责分层清晰(audio-io / processors /
  core / cli)、`EchoProcessor` trait + `ProcessorChain` 抽象合理、FFI 边界规范、vendored sonora 经核
  **无可达 panic / 无明显 UB**、集成面干净。问题是**局部债**而非结构性腐败:几处实时热路径低效、两个
  巨石文件、打包未完成、一个前后端耦合取舍。
- **理由(对照 §0 威胁模型与 §10 结论)**:重写会丢弃已**验证正确**的 DSP 集成(sonora/AEC3/LocalVQE/
...
| **P3 实时音质/稳定** | 核心产品质量 | PERF-1, QUAL-1, QUAL-3, QUAL-4, ROB-2, SON-1 | `phase-3/realtime` | chain.rs 稳态**零分配**(加计数 allocator 测试)+ rubato **有状态**重采样 + 立体声保留(加块边界连续性测试);LocalVqe 用 VecDeque;stats 在线分桶;诊断 writer 正确 join;sonora process 结果不再吞 |
| **P4 安全/健壮** | 加固(多为小改,可批量) | SEC-1, SEC-2, SEC-3, SEC-4, SEC-5, SEC-6, ROB-1, ROB-3, ROB-4, QUAL-2, SON-2, SON-3, DOC-1 | `phase-4/harden` | open_url 校验 scheme;nvafx 内置 pin 优先;唯一/私有临时文件(O_EXCL);localvqe 库不搜 CWD;模型下载校验 SHA;Tauri 长命令 `spawn_blocking`+超时;锁去毒化;`flatten` 不吞行;frame_size 用 u64;sonora 热路径 `assert!`→`debug_assert!` 审查 + 进 CI;pin 哈希集中 |
...
- **新增步骤 P0.0(排在 P0 最前,独立小提交)——把基线刷绿**。完整清单:
  ...
  | # | 位置 | lint | 修法 |
  |---|------|------|------|
  | 1 | `processors/src/sonora_aec3.rs:61` | items_after_test_module | 把 `mod tests` 移到文件末尾(其后的 `SonoraAec3` 等生产代码挪到 test 模块之前) |
...
- ✅ **无可达生产 panic**:首方 Rust 唯一的 `panic!`(`realtime.rs:3236`)在 `#[test]` 内;与 §10 sonora 结论一致。
...
- ✅ **执行前置就绪**:三个 workspace 各有 `Cargo.lock`(root / app/src-tauri / vendor/sonora —— `cargo audit`

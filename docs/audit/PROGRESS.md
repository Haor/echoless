...
| ARCH-1 | P2 split | doing | `3c611a6` | Started mechanical split by moving runtime stdin control parsing/dispatch into `realtime/control.rs` with no behavior changes. Verified with `cargo fmt -p echoless-cli --check`; `cargo test -p echoless-cli runtime_control --locked`; `cargo clippy --workspace --all-targets --locked -- -D warnings`; `cargo test --workspace --locked`. Remaining: split diagnostics, stats, resampling, device selection, and `main.rs` command helpers |

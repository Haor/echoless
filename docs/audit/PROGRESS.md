...
| ARCH-1 | P2 split | doing | `3c611a6`, `97ca670`, `53d6223`, `532c123`, `5108bf5`, `8a232f0`, `e47f3e3`, `411e7b6` | Started mechanical split by moving runtime stdin control parsing/dispatch into `realtime/control.rs`, device I/O linear resamplers into `realtime/resample.rs`, live status aggregation into `realtime/stats.rs`, diagnostic recorder/writer handling into `realtime/diagnostics.rs`, device selection/doctor helpers into `realtime/devices.rs`, `probe-delay` CLI/DSP helpers into `probe_delay.rs`, config validation helpers into `config_validate.rs`, and RTX AEC nvafx helpers into `nvafx_install.rs` with no behavior changes. Verified with targeted control/resampler/stats/diagnostic/device/probe/config/nvafx tests plus `cargo clippy --workspace --all-targets --locked -- -D warnings` and `cargo test --workspace --locked`. Remaining: thin `main.rs` CLI dispatcher/offline/run helper boundary if needed |

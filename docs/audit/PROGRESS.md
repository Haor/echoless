...
| ARCH-1 | P2 split | doing | `3c611a6`, `97ca670`, `53d6223`, `532c123` | Started mechanical split by moving runtime stdin control parsing/dispatch into `realtime/control.rs`, device I/O linear resamplers into `realtime/resample.rs`, live status aggregation into `realtime/stats.rs`, and diagnostic recorder/writer handling into `realtime/diagnostics.rs` with no behavior changes. Verified with targeted control/resampler/stats/diagnostic tests plus `cargo clippy --workspace --all-targets --locked -- -D warnings` and `cargo test --workspace --locked`. Remaining: split device selection and `main.rs` command helpers |

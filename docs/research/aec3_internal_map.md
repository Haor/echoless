# aec3-core 内部地图(集成前调研)

> 目的:在决定"如何把 aec3-core 集成进 Echoless"之前,摸清它的内部结构、可调参数、
> 延迟/漂移链路、已知坑点。**研究先行,不盲目搬代码。**
>
> 源码版本:`dignifiedquire/aec3` rev `aacadf0`(= 我们 `Cargo.lock` 锁定的 commit)。
> 本地路径:`~/.cargo/git/checkouts/aec3-c0cc0c3c8411109a/aacadf0/crates/aec3-core`。
> 下文行号引用均以该副本为准(`crates/aec3-core/src/...` 省略前缀写作 `文件:行`)。
>
...
aec3-core 是 WebRTC AEC3 的**逐文件 1:1 Rust 端口**,架构忠实、能用,但:

- **真正的"调参旋钮"(tail 长度、抑制激进度、延迟搜索范围)全在 `EchoCanceller3Config`,
  而高层 `aec3` crate 把它们 100% 锁死,只放行 `transparent_mode` 一个字段。**
...
要么 fork 高层 aec3 开个 config 注入口,要么绕过高层直接驱动 `BlockProcessor`。**
...
`EchoCanceller3` 这一层在**上层 `aec3` crate**里、且是 `pub(crate)`。aec3-core 对外暴露的处理入口是
...
- 唯一构造点 `audio_processing_impl.rs:1280-1282`:
  ...
  ```
- 高层 `EchoCanceller`(`aec3/src/config.rs:175-194`)只有 2 个字段:`enforce_high_pass_filtering`(走独立 HPF 子模块,**不进 aec3 config**)+ `transparent_mode`。
...
- `aec3::config::EchoCanceller3Config` 在 aec3 公共 API 里**没有任何注入 hook**。
...
- → 我们现在 `aec3.rs` 里填 `initial_delay_ms` 基本没用(走高层默认路径);要么靠内部自动估(已验证可行,自动估出 48ms),要么 fork 开 `use_external_delay_estimator` 自己做粗对齐。
...
## 6. 高层 aec3 在中间做了什么(绕过它就要自己补)

高层 `EchoCanceller3`(`aec3/src/echo_canceller3.rs`)在调 `BlockProcessor` 前后做的"胶水":
...
`aec3.rs:177-185` 读了 `statistics()` 的字段,核实 `audio_processing_impl.rs:732-749`:
...
- **上限被锁死的根因**:高层 aec3 不放行 `EchoCanceller3Config`。要拿 tail 长度/抑制激进度/延迟搜索范围/导出线性 AEC 输出(喂 LocalVQE 的关键),**必须能改这个 config**。
- 三条路:

| 方案 | 做法 | 成本 | 拿到的上限 |
|---|---|---|---|
| **A. fork 高层 + 开 config 注入口** | vendor 整个 aec3 workspace 改 path,在 `AudioProcessingBuilder` 加一个 `.aec3_config(EchoCanceller3Config)` setter,改 `audio_processing_impl.rs:1280` 让它接受外部 config | **低**(改几十行,胶水全复用:量程/三带/分帧/队列都不用自己写) | **全部 config 可调**,且自动避开坑 #1/#6 等 |
...
- 两者都需要 vendor 源码(git 依赖无法改),所以第一步动作相同:**vendor aec3 workspace 进 `echoless/vendor/`,改 path 依赖**。

### 9.3 落地步骤(若采纳 A)

1. vendor:把 aec3 workspace(去 cpp/fuzz/ffi/bench/sys)拷进 `echoless/vendor/aec3/`,`echoless-processors` 改 path 依赖。
...
3. `EchoCanceller3Config` 在 aec3 公共 API 重导出(`pub use aec3::config::EchoCanceller3Config`)。
4. `aec3.rs::configure` 把我们的 TOML 参数(tail_ms→length_blocks、suppressor 激进度、num_filters、export_linear_aec_output…)映射进 `EchoCanceller3Config`,构造前注入。
5. 复核 §7 统计来源;按需在高层 `statistics()` 暴露 `has_clockdrift`/原始 delay/jitter(下沉 B 的前哨)。
6. 回归:重跑合成 ERLE 测试 + 真实录音 eval,确认改 tail/激进度后指标变化符合预期。

> 注:本项目本地自用、无 license 顾虑([[aec-self-use-no-license]]),fork/改 aec3 源码无障碍。
...
vendored fork(`echoless/vendor/aec3`)+ config 注入口已落地并跑通。合成回归测试
(`echoless-processors/tests/{echo_cancellation,aec3_direct}.rs`)挖出几条**关键经验**:
...
→ 结论:**aec3 AEC3 工作正常**。任何"AEC 没效果"的合成测试,先检查激励是否非平稳。

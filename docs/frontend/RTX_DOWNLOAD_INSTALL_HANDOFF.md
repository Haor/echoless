...
| **`nvafx_download_install`** | `runtime_dir?` | ✅ 前端已接 | shell `nvafx download-install [--runtime-dir D] --json`(**待后端实现该子命令**) |

`nvafx_download_install` 已改为 shell `echoless nvafx download-install --json`,
期望该子命令打印 `{ok, report}` doctor JSON 到 stdout。后端实现该子命令后即端到端生效;
未实现前 CLI 非 0 退出,错误经 stderr 透传给前端。请实现该子命令(见 §4)。

## 4. 请实现:公共 release 下载安装

### 推荐形态:新增一个 CLI 子命令(与现有 `nvafx install` / `doctor` 一致)
...
1. 跑 `nvafx doctor` 取 `selected_arch`(或 `nvidia-smi --query-gpu=compute_cap` 映射)。
...
2. 从公共 release 下载:
   ...
   - 下载源默认上面的 release base;`--tag` 可覆盖。建议也读 `manifest.json` 拿
     `download_url` / `sha256`,而不是硬编码 URL。
3. 校验 SHA256(对照 `SHA256SUMS.txt` 或 manifest)。
4. 复用现有 install 逻辑解压到 `runtime_dir`(缺省 `%LOCALAPPDATA%\Echoless\nvafx\2.1.0`),
   写 `echoless-runtime-install-manifest.json`。
...
- 后端: 实现 `nvafx download-install`(本文 §4),前端随后把 stub 换成 shell 它。

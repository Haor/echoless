...
- macOS 还需要随包 `echoless-process-tap-poc` helper,或设置
  ...
  当前 app 层已在能定位 helper 时给 CLI 子进程注入该 env;打包时仍需让 helper 成为已签名 bundle 内容。
...
- 生产打包应走 Tauri 官方 sidecar 固定文件名 / target-triple 规则,不要靠模糊文件扫描。
...
## 下一会话建议
- 后端/Codex:§6.1 sidecar + §6.2 LocalVQE assets,然后 `pnpm tauri build` 出端到端安装包。

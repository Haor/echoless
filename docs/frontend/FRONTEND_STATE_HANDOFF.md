...
1. 把 `echoless` CLI 作为 **sidecar 打进 bundle**,并让 app 在生产环境解析到它(§6.1)。
2. **bundle LocalVQE assets**(model + 动态库)并加一个 assets 探针,让 LocalVQE 默认 READY(§6.2)。
3. 实现 **`nvafx download-install`** CLI 子命令(仓库已公开,见 RTX 专题文档 §4)。
...
Events:
- `echoless://status` — `started` 事件(backend/sr/frame/session_dir)+ 此后每条 `status`(dBFS、波形、队列、drop、latency、diverged…)。字段见 `FRONTEND_AGENT_HANDOFF.md`。
- `echoless://exit` — sidecar 退出。
- `echoless://log` — stderr 行。

TS 镜像:`app/src/types.ts`;调用层:`app/src/api.ts`;配置 TOML 生成:`api.ts` 的 `buildConfigToml()`(顶层 mic/reference/output/sample_rate/frame_ms/reference_channels + 可选 `[diagnostics]` + `[[chain]] kind + params`)。
...
- 把对应平台的 `echoless`(Windows 需 `realtime`/cpal 特性、随附 `localvqe.dll`+GGML;macOS 同理)
  作为 Tauri **externalBin / resource** 打进 bundle。
- 让 `echoless_bin()` 在生产解析到 bundled 路径(例:启动时用 `app.path().resource_dir()` 求出 CLI 路径并 `set_var("ECHOLESS_BIN", ...)`,dev 行为不变)。
- 校验:打包产物里 `devices --json` / `run` 能跑(当前 dev 用 `ECHOLESS_BIN` 或 `target/release/echoless`)。

### 6.2 LocalVQE bundled assets
- bundle `models/localvqe-v1.3-4.8M-f32.gguf` + 平台动态库(`localvqe.dll` / `liblocalvqe*.dylib` + GGML modules)。
- 加一个探针命令(建议 contract,见 ENGINE_RUNTIME_WIZARD_GUIDANCE.md `LocalVqeAssets`):返回 bundled model/library 绝对路径。
- 前端会据此把 LocalVQE 显示为 READY 并自动写 `model = <bundled path>`(目前未拿到探针前用文件选择器,见 EnginePage)。
...
- `tauri.conf.json` 目前 `bundle.targets="all"`,无 `externalBin`/`resources` —— 6.1/6.2 需在此补。
...
ECHOLESS_BIN=/abs/path/to/echoless pnpm tauri dev   # 或先 cargo build -p echoless-cli --release
pnpm tauri build           # 打包(待 §6.1/6.2 接好 sidecar/assets 后才完整可用)
```
...
## 下一会话建议
- 后端/Codex:§6.1 sidecar + §6.2 LocalVQE assets + RTX `download-install`,然后 `pnpm tauri build` 出端到端安装包。

...
2. LocalVQE assets 探针已扩展为 model + native readiness;模型下载到
   `<brand data root>/localvqe/models`,native runtime 随包分发(2026-07-05 定案)。
...
| LocalVQE:选择 + 下载 `.gguf` + native readiness | ✅(模型走 HF 下载;native 随包) |
...
### 6.2 LocalVQE assets(2026-07-05 定案)
- 模型不进 bundle,首次使用从 HF 下载到品牌数据目录;native runtime 随包分发
  (prepare:tauri-assets 拷入 resources,发布用 `--require-localvqe-assets` 缺库即 fail)。

...
# 字节稳定 = 授权跨 dev 重启存活。
FINGERPRINT=$(cat "$SCRIPT_DIR/Sources/main.swift" "$SCRIPT_DIR/Info.plist" "$0" | shasum -a 256 | cut -d' ' -f1)
...
# 否则 ad-hoc。
if security find-identity -v -p codesigning 2>/dev/null | grep -q "Echoless Dev"; then

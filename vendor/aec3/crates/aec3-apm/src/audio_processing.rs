...
/// use aec3_apm::config::{EchoCanceller, NoiseSuppression};
///
/// let config = Config {
...
pub struct AudioProcessingBuilder {
    ...
    echo_detector: bool,
    // Echoless: 可选的底层 AEC3 调参注入(None=用上游默认)。
    aec3_config: Option<crate::EchoCanceller3Config>,
}
...
impl AudioProcessingBuilder {
    ...
    /// Echoless: 注入自定义底层 [`EchoCanceller3Config`](crate::EchoCanceller3Config)。
    ///
    /// 上游高层 API 只放行 `transparent_mode`,其余上百个调参字段(echo tail 长度、
    /// suppressor 激进度、延迟搜索范围、`export_linear_aec_output` 等)被锁死。此入口
    /// 让调用方直接提供完整 config。`transparent_mode` 仍由高层 `EchoCanceller` 旋钮叠加。
    /// 见 research/aec3_internal_map.md §2/§9。
    pub fn aec3_config(mut self, config: crate::EchoCanceller3Config) -> Self {
    ...
    pub fn build(self) -> AudioProcessing {
        ...
        // Echoless: 在 initialize_with_config 重建 echo_controller 之前注入自定义 AEC3 config。
        inner.set_aec3_config_override(self.aec3_config);
